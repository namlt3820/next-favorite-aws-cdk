...
export const traktGetDetailMovieConstruct = ({
  scope,
  env,
}: {
  scope: Construct;
  ...
  const traktGetDetailMovieFunction = new lambda.Function(
    scope,
    `NF-TraktGetDetailMovieFunction-${env}`,
    {
      runtime: lambda.Runtime.NODEJS_20_X,
      handler: "handler.handler",
      code: lambda.Code.fromAsset("lambdas/trakt/get-detail-movie/dist"),
      environment: {
  ...
  return { traktGetDetailMovieFunction };
};
