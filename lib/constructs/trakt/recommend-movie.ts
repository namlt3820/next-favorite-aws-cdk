...
export const traktRecommendMovieConstruct = ({
  scope,
  env,
  favoriteTableName,
  ignoreTableName,
  ...
  const traktRecommendMovieFunction = new lambda.Function(
    scope,
    `NF-TraktRecommendMovieFunction-${env}`,
    {
      runtime: lambda.Runtime.NODEJS_20_X,
      handler: "handler.handler",
      code: lambda.Code.fromAsset("lambdas/trakt/recommend-movie/dist"),
      environment: {
  ...
  return { traktRecommendMovieFunction };
};
